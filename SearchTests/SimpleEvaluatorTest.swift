...
class PushOneTimeScenarioTest: XCTestCase {
    ...
    func test_OneTimeScenarioInitializedWithScoreFunction() {
        ...
        let six_evaluator = PushOneTimeScenario(name:"Always six",scorer:return_six)
        let should_be_six = six_evaluator.score(a1)
        XCTAssertTrue(should_be_six == 6.0, "expected 6.0, got \(should_be_six)")
        XCTAssertTrue(a1.scores[six_evaluator.name] == 6.0, "expected 6.0, got \(a1.scores[six_evaluator.name])")
    }
    ...
    func test_LengthCountScenarioReturnsValue() {
        let a1 = PushAnswer(length: 11, commands: ["x", "y"], otherTokens: [], commandRatio: 1.0)
        let length_evaluator = PushOneTimeScenario(name: "number of tokens") {(a:PushAnswer) -> Double in
    ...
    func test_LengthCountShouldWriteToAnswerScores() {
        ...
        XCTAssertTrue(a1.scores[length_evaluator.name]! == 6.0, "expected score to be 6.0  not \(a1.scores[length_evaluator.name])")
    }
    
    func test_LengthCountShouldNotOverwriteItself() {
        var a1 = PushAnswer(length: 6, commands: ["x", "y"], otherTokens: [], commandRatio: 1.0)
        let length_evaluator = PushOneTimeScenario(name:"numbr of tokens") {(a:PushAnswer) -> Double in
            return Double(a.template.count)
        }
        a1.scores[length_evaluator.name] = 99.99
        length_evaluator.score(a1)
        XCTAssertTrue(a1.scores[length_evaluator.name]! == 99.99, "expected score to be 99.99  not \(a1.scores[length_evaluator.name]!)")
    }
